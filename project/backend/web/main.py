"""
EIMS — Web Backend (Super Admin, IA, EO, Teacher Admin)
"""
import logging
...
app.include_router(institutions_router,  prefix=f"{PREFIX}/institutions",   tags=["Institutions"])
...
app.include_router(admin_router,         prefix=f"{PREFIX}/admin",          tags=["Admin Tools"])
app.include_router(ed_officials_router,  prefix=f"{PREFIX}/ed-officials",   tags=["Ed Officials"])
app.include_router(faculty_router,       prefix=f"{PREFIX}/faculty",        tags=["Faculty"])
app.include_router(super_admin_router,   prefix=f"{PREFIX}/super-admin",    tags=["Super Admin"])
app.include_router(ws_router,            prefix="/ws",                      tags=["WebSockets"])

@app.get("/health")
async def health(): return {"status": "healthy", "service": "web-api"}
